...
layui.define(['laypage', 'fly', 'element', 'flow','upload','form','layer'], function(exports){
  ...
  chat.click(function () {
    layer.open({
      ...
      yes:function(index,layero){
        $.post(
            '/letter/send',
            {"toName":$('#toName').val(),"content":$('#whisper').val()},
            function (data) {
              data = $.parseJSON(data);
              if(data.code == 0){
  ...
  if($('.upload-img')[0]){
    layui.use('upload', function(upload){
      ...
      var url = 'http://upload-z1.qiniup.com?token=' + $('#token').val();
      var key = $('#key').val();
      upload.render({
        elem: '.upload-img'
        ,url: url
        ...
        ,data: {
         ...
         ,before: function(obj){
          obj.preview(function(index, file, result){
            avatarAdd.find('#image').attr("src",result);
          });
        }
        ,done: function(res){
          if(res.code == 0){
    ...
    $.post(
        ...
        function (res) {
          if (res.code == 1) {
            ...
            $('#L_nowpass').focus();
            $('#L_nowpass').addClass("layui-form-danger");
            setTimeout(function () {
              $('#L_nowpass').removeClass("layui-form-danger");
            }, 2000);
          } else if (res.code == 2) {
            ...
            $('#L_pass').focus();
            $('#L_pass').addClass("layui-form-danger");
            setTimeout(function () {
              $('#L_pass').removeClass("layui-form-danger");
            }, 2000);
          } else if (res.code == 0) {
            layer.msg(res.msg, {
              icon: 6,
    ...
    $.post(
        ...
        function (res) {
         if (res.code == 0) {
            ...
            window.location.href="/user/setting/"+$('#userId').val();
          }
        }, 'json');
    return false;
  });
